...
class Network_fc_bin_W(nn.Module):
    ''' Construct a network with a fully-connected architecture which has binary synapses and a full-precision activation function
    The network is trained in the prototypical settings of EP
    '''
    def __init__(self, args):
        ...
        with torch.no_grad():
            for i in range(len(args.layersList)-1):
                ...
                self.W[-1].weight.copy_(self.weightOffset*torch.sign(self.W[-1].weight.data))


    def stepper(self, s, target = None, beta = 0):
        '''
    ...
    def forward(self, s, seq = None,  beta = 0, target = None):
        '''
        Relaxation function
        '''
        ...
        with torch.no_grad():
            if beta == 0:
                ...
                # Nudged phase
                for t in range(Kmax):
                        s = self.stepper(s, target = target, beta = beta)
...
class Network_conv_bin_W(nn.Module):
    ...
    def stepper(self, s, data, inds, target = None, beta = 0.): 
